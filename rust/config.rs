...
pub struct GrpcProxyFilter;

impl Filter for GrpcProxyFilter {
    fn filter(&self, record: &log::Record<'_>) -> Response {
        //if record.module_path().unwrap_or("").starts_with(crate::consts::targets::LM) {
        if record
            .module_path()
            .unwrap_or("")
            .starts_with("grpc_proxy::")
        {
            Response::Neutral
        } else {
            Response::Reject
...
pub struct ProxyPaths {
    pub paths: HashMap<Arc<str>, Vec<Addr>>,
}

impl std::ops::Deref for ProxyPaths {
    type Target = HashMap<Arc<str>, Vec<Addr>>;
