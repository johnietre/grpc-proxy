...
Have a nice day! :]"
    ...
    match cmd {
        Commands::Config(cc) => run_config_command(args, cc).await,
        Commands::Start(sc) => run_start_command(args, sc).await,
        Commands::Get(gc) => run_get_command(args, gc).await,
        Commands::Refresh(rc) => run_refresh_command(args, rc).await,
        Commands::Shutdown(sc) => run_shutdown_command(args, sc).await,
    }
}
...
async fn run_start_command(_: CliArgs, sc: StartCommand) {
...
fn setup_logging(sc: &StartCommand) -> Option<log4rs::Handle> {
    ...
    use log4rs::config::{Appender, Config as LogConfig, Root, load_config_file};

    let target_val = env::var(LOG_TARGET_ENV_NAME).unwrap_or_default();
    /*
    let filters = target_val.split(',')
        .map(|s| Box::new(TargetFilter(String::from(s))) as Box<dyn Filter>);
    */
    let filters = TargetsFilter(target_val.split(',').map(String::from).collect());
    ...
    let config = if let Some(lc_path) = sc.log_config.as_ref() {
        ...
        let res = LogConfig::builder()
            //.appender(Appender::builder().filters(filters).build("appender", appender))
            .appender(Appender::builder().filter(Box::new(filters)).build("appender", appender))
